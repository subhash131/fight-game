...
function animate({
  ...
  requestAnimationFrame(() => animate({ backgroundCanvas, c, canvas, dpr }));
}
...
export const startRendering = async ({
  ...
  try {
    const backgroundCanvas = await renderStaticLayers({ canvas });
    if (!backgroundCanvas) {
      console.error("Failed to create the background canvas");
      return;
    }
    animate({ backgroundCanvas, c: context, canvas, dpr });
  } catch (error) {
